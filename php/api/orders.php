...
switch ($method) {
    case 'GET':
        ...
        if ($id) {
            $stmt = $db->prepare('
                ...
                LEFT JOIN tnsat_delivery_types dt ON s.delivery_type_id = dt.id
                WHERE o.id = ?
            ');
            $stmt->execute([$id]);
            $row = $stmt->fetch();
    ...
    case 'POST':
        ...
        try {
            if ($resellerId) {
                $stmt = $db->prepare('SELECT * FROM tnsat_resellers WHERE id = ? FOR UPDATE');
                ...
                $buyerTable = 'tnsat_resellers';
                $buyerId = $resellerId;
            } else {
                $stmt = $db->prepare('SELECT * FROM tnsat_clients WHERE id = ? FOR UPDATE');
                ...
                $buyerCredits = floatval($buyer['credits']);
                $buyerTable = 'tnsat_clients';
                $buyerId = $clientId;
            }
            
            $stmt = $db->prepare('SELECT * FROM tnsat_services WHERE id = ? FOR UPDATE');
            ...
            if ($resellerId) {
                ...
                if ($mode !== 'all') {
                    $vStmt = $db->prepare('SELECT 1 FROM tnsat_reseller_service_visibility WHERE service_id = ? AND reseller_id = ?');
                ...
                $ovStmt = $db->prepare('SELECT price_credits FROM tnsat_reseller_service_prices WHERE service_id = ? AND reseller_id = ?');
            ...
            if ($saleType === 'stock') {
                $stmtChk = $db->prepare('SELECT COUNT(*) FROM tnsat_product_keys WHERE service_id = ? AND status = "available"');
                ...
                if (intval($stmtChk->fetchColumn()) <= 0) {
                    ...
                    try {
                        ...
                        $logDb->prepare('INSERT INTO tnsat_notifications (id, client_id, reseller_id, type, message, is_read) VALUES (?, NULL, NULL, ?, ?, 0)')
                              ->execute([$nid, 'stock_out_attempt', $msg]);
                        // Persist structured log row
                        $aid = bin2hex(random_bytes(16));
                        $logDb->prepare('INSERT INTO tnsat_stock_out_attempts (id, service_id, client_id, reseller_id, attempted_credits) VALUES (?, ?, ?, ?, ?)')
            ...
            $stmt = $db->prepare('INSERT INTO tnsat_orders (id, client_id, reseller_id, service_id, credits_used, duration_months, status, delivery_type_id, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
            ...
            if ($service['stock'] !== null) {
                $stmt = $db->prepare('UPDATE tnsat_services SET stock = stock - 1 WHERE id = ?');
                $stmt->execute([$serviceId]);
            }
            ...
            $stmt = $db->prepare('INSERT INTO tnsat_point_transactions (id, client_id, reseller_id, type, amount, balance_after, description, order_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
            ...
            if ($availableKey) {
                ...
                $stmt = $db->prepare('UPDATE tnsat_product_keys SET status = "assigned", order_id = ?, assigned_at = NOW(), reseller_note = ? WHERE id = ?');
                ...
                $stmt = $db->prepare('UPDATE tnsat_orders SET credentials = ?, status = ?, fulfilled_at = NOW() WHERE id = ?');
                ...
                $stmtRem = $db->prepare('SELECT COUNT(*) FROM tnsat_product_keys WHERE service_id = ? AND status = "available"');
    ...
    case 'PUT':
        ...
        if ($action === 'fulfill') {
            ...
            $stmt = $db->prepare('SELECT client_id, reseller_id, service_id FROM tnsat_orders WHERE id = ?');
            $stmt->execute([$id]);
            $order = $stmt->fetch();
            ...
            $stmt = $db->prepare('UPDATE tnsat_orders SET credentials = NULL, status = ?, fulfilled_at = NULL WHERE id = ?');
            $stmt->execute(['pending', $id]);
            jsonResponse(['success' => true, 'status' => 'pending']);
        } elseif ($action === 'cancel') {
            $db->beginTransaction();
            try {
                $stmt = $db->prepare('SELECT * FROM tnsat_orders WHERE id = ? FOR UPDATE');
                $stmt->execute([$id]);
                $order = $stmt->fetch();
                ...
                if ($resellerId) { $buyerTable = 'tnsat_resellers'; $buyerId = $resellerId; }
                else { $buyerTable = 'tnsat_clients'; $buyerId = $clientId; }

                $stmt = $db->prepare("UPDATE {$buyerTable} SET credits = credits + ? WHERE id = ?");
                $stmt->execute([$creditsUsed, $buyerId]);

                $stmt = $db->prepare('SELECT stock FROM tnsat_services WHERE id = ?');
                $stmt->execute([$order['service_id']]);
                $service = $stmt->fetch();
            ...
            $params[] = $id;
            $stmt = $db->prepare('UPDATE tnsat_orders SET ' . implode(', ', $updates) . ' WHERE id = ?');
            $stmt->execute($params);
    ...
    case 'DELETE':
        ...
        $stmt = $db->prepare('DELETE FROM tnsat_orders WHERE id = ? AND status IN (?, ?, ?)');

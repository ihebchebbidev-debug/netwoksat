...
switch ($method) {
    case 'GET':
        if ($id) {
            $stmt = $db->prepare('SELECT c.*, (SELECT COUNT(*) FROM tnsat_services s WHERE s.category = c.name) AS product_count FROM tnsat_categories c WHERE c.id = ?');
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            ...
            $stmt = $db->query('SELECT c.*, (SELECT COUNT(*) FROM tnsat_services s WHERE s.category = c.name) AS product_count FROM tnsat_categories c ORDER BY sort_order ASC, created_at ASC');
            $rows = $stmt->fetchAll();

            $listed = [];
            if ($resellerId) {
                $vstmt = $db->prepare('SELECT category_id FROM tnsat_reseller_category_visibility WHERE reseller_id = ?');
    ...
    case 'POST':
        ...
        // Check duplicate
        $chk = $db->prepare('SELECT id FROM tnsat_categories WHERE name = ?');
        $chk->execute([$name]);
        if ($chk->fetch()) jsonResponse(['error' => 'Category already exists'], 409);
        $id = bin2hex(random_bytes(16));
        $stmt = $db->prepare('INSERT INTO tnsat_categories (id, name, image_url, sort_order) VALUES (?, ?, ?, ?)');
    ...
    case 'PUT':
        ...
        $old = $db->prepare('SELECT name FROM tnsat_categories WHERE id = ?');
        $old->execute([$id]);
        $oldRow = $old->fetch();
        $stmt = $db->prepare('UPDATE tnsat_categories SET name = ?, image_url = ?, sort_order = ? WHERE id = ?');
        $stmt->execute([$name, $data['image_url'] ?? '', intval($data['sort_order'] ?? 0), $id]);
        // Rename category on all services
        if ($oldRow && $oldRow['name'] !== $name) {
            $upd = $db->prepare('UPDATE tnsat_services SET category = ? WHERE category = ?');
    ...
    case 'DELETE':
        ...
        $cat = $db->prepare('SELECT name FROM tnsat_categories WHERE id = ?');
        $cat->execute([$id]);
        $catRow = $cat->fetch();
        if ($catRow) {
            $upd = $db->prepare('UPDATE tnsat_services SET category = NULL WHERE category = ?');
            $upd->execute([$catRow['name']]);
        }
        $stmt = $db->prepare('DELETE FROM tnsat_categories WHERE id = ?');
        $stmt->execute([$id]);
        jsonResponse(['success' => true]);
        break;
}

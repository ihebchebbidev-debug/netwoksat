...
switch ($method) {
    case 'GET':
        $db = getDB();
        if ($id) {
            $stmt = $db->prepare('SELECT * FROM tnsat_clients WHERE id = ?');
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            ...
            if ($page !== null) {
                $countStmt = $db->prepare("SELECT COUNT(*) FROM tnsat_clients{$where}");
                $countStmt->execute($params);
                $total = intval($countStmt->fetchColumn());

                $offset = ($page - 1) * $limit;
                $stmt = $db->prepare("SELECT * FROM tnsat_clients{$where} ORDER BY created_at DESC LIMIT {$limit} OFFSET {$offset}");
                $stmt->execute($params);
                $rows = $stmt->fetchAll();
                foreach ($rows as &$r) castClientRow($r);
                jsonResponse(['data' => $rows, 'total' => $total, 'page' => $page, 'limit' => $limit]);
            } else {
                $stmt = $db->prepare("SELECT * FROM tnsat_clients{$where} ORDER BY created_at DESC");
                $stmt->execute($params);
                $rows = $stmt->fetchAll();
                foreach ($rows as &$r) castClientRow($r);
                jsonResponse($rows);
    ...
    case 'POST':
        ...
        if ($action === 'batch_delete') {
            ...
            $stmt = $db->prepare("DELETE FROM tnsat_clients WHERE id IN ({$placeholders})");
            $stmt->execute($ids);
        ...
        if ($action === 'batch_toggle_active') {
            ...
            $db = getDB();
            $placeholders = implode(',', array_fill(0, count($ids), '?'));
            $stmt = $db->prepare("UPDATE tnsat_clients SET is_active = NOT is_active WHERE id IN ({$placeholders})");
            $stmt->execute($ids);
        ...
        $stmt = $db->prepare('SELECT id FROM tnsat_clients WHERE email = ?');
        $stmt->execute([$email]);
        if ($stmt->fetch()) {
        ...
        $stmt = $db->prepare('INSERT INTO tnsat_clients (id, name, email, password, credits) VALUES (?, ?, ?, ?, ?)');
    ...
    case 'PUT':
        ...
        if ($action === 'add_credits') {
            ...
            $stmt = $db->prepare('UPDATE tnsat_clients SET credits = credits + ? WHERE id = ?');
            $stmt->execute([$credits, $id]);

            $stmt = $db->prepare('SELECT credits FROM tnsat_clients WHERE id = ?');
            $stmt->execute([$id]);
            $updated = $stmt->fetch();
            $txId = bin2hex(random_bytes(16));
            $desc = $note ? "Crédits ajoutés par admin — $note" : 'Crédits ajoutés par admin';
            $stmt = $db->prepare('INSERT INTO tnsat_point_transactions (id, client_id, type, amount, balance_after, description) VALUES (?, ?, ?, ?, ?, ?)');
        ...
        if ($action === 'empty_credits') {
            ...
            if ($currentCredits > 0) {
                $stmt = $db->prepare('UPDATE tnsat_clients SET credits = 0 WHERE id = ?');
                $stmt->execute([$id]);
                $txId = bin2hex(random_bytes(16));
                $stmt = $db->prepare('INSERT INTO tnsat_point_transactions (id, client_id, type, amount, balance_after, description) VALUES (?, ?, ?, ?, ?, ?)');
        ...
        if ($action === 'toggle_active') {
            $stmt = $db->prepare('UPDATE tnsat_clients SET is_active = NOT is_active WHERE id = ?');
            $stmt->execute([$id]);
            jsonResponse(['success' => true]);
            break;
        }
        ...
        $stmt = $db->prepare('SELECT id FROM tnsat_clients WHERE email = ? AND id != ?');
        $stmt->execute([$email, $id]);
        if ($stmt->fetch()) {
            jsonResponse(['error' => 'Email already exists'], 400);
        }
        
        $stmt = $db->prepare('UPDATE tnsat_clients SET name=?, email=?, password=?, credits=? WHERE id=?');
    ...
    case 'DELETE':
        if (!$id) jsonResponse(['error' => 'ID required'], 400);
        $db = getDB();
        $stmt = $db->prepare('DELETE FROM tnsat_clients WHERE id = ?');
        $stmt->execute([$id]);
        jsonResponse(['success' => true]);
        break;
